        ...
        if self.initialized:
            for split_name in ['train', 'test']:
                ...
                for i, norb_example in tqdm(iterable=enumerate(self.data[split_name]),
                    ...
                    cat_dir = join(split_dir, category)
                    if not exists(cat_dir):
                        makedirs(cat_dir)

                    image_lt_path = join(cat_dir, '{:06d}_{}_{:02d}_lt.jpg'.format(i, category, instance))
                    image_rt_path = join(cat_dir, '{:06d}_{}_{:02d}_rt.jpg'.format(i, category, instance))
