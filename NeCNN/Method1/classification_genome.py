...
class ClassificationGenomeConfig(object):
    """
    Default Genome of python-neat with a modified crossover step.
    Here the child inherits the connections from both parents with probability (inherit_disjoint_coefficient).
    Also the child gets all nodes that are associated to a connection.
    """
...
class ClassificationGenome(object):
    ...
    def mutate_add_node(self, config):
        ...
        if hasattr(conn_to_split, "weight"):
            self.add_connection(config, new_node_id, o, conn_to_split.weight, True)
        else:
            self.add_connection(config, new_node_id, o, 1.0, True)
